...
#--- 2. head()와 tail()로 데이터 확인
df.head()
df.tail()

#--- 3. 데이터 전처리
'''
온라인으로 수집한 데이터는 다양한 이유로 중복이 생성
웹사이트에서 전송 버튼을 여러 번 누르거나, 새로 고침을 하거나
네트워크나 UX 관련 오류 문제가 발생하면 중복 발생
이런 중복 데이터가 있으면 빈도 분석이 제대로 되지 않기 때문에 중복 입력값을 제거
'''
...
# 같은 의미의 단어를 하나로 통일
# 예를 들어 python을 파이썬으로
df['text'] = df['text'].str.replace('python', '파이썬').str.replace('pandas', '판다스').str.replace('javascript', '자바스크립트').str.replace('java', '자바').str.replace('react', '리액트').str.replace('django', '장고')

#--- 4. 문자열 분리로 관심 강의 분리
'''
이 댓글 이벤트에는 ‘관심강의’라는 텍스트가 있음
대부분 관심강의 뒤에 강의명을 적었으므로 ‘관심강의’를 기준으로 이전 텍스트는 제거하고 
이후 텍스트를 새로운 변수(course)에 담고
'관심 강의', '관심 강좌'에 대해서도 똑같이 전처리

':' 특수문자를 빈 문자로 변경

문자열 분리(split)로 관심강의를 분리
'''
df['course'] = df['text'].apply(lambda x: x.split('관심강의')[-1])
df['course'] = df['text'].apply(lambda x: x.split('관심 강의')[-1])
df['course'] = df['text'].apply(lambda x: x.split('관심 강좌')[-1])
...
#--- 5. 빈도 계산을 위해 텍스트 데이터를 벡터화
# BoW 단어 가방에 단어를 토큰화해서 담아주기
...
'''
전체 단어 가방에서 해당 리뷰마다 등장하는 단어에 대한 빈도 수
행은 각 리뷰를 의미하고, 그 중 0 값은 해당 단어가 등장하지 않음을 의미

그렇기에 BoW로 벡터화하면 희소 행렬이 만들어지게 되는 단점
희소 행렬은 행렬의 대부분 원소가 0인 행렬을 의미

희소 행렬 활용 분야는
웹 검색 엔진: 웹 페이지 간의 연결 관계를 나타내는 행렬은 희소 행렬 형태
소셜 네트워크: 사용자 간의 연결 관계를 나타내는 행렬
추천 시스템: 사용자-아이템 간의 평점 데이터
자연어 처리: 문서-단어 간의 출현 빈도를 나타내는 행렬

대부분의 원소가 0이라서 0이 아닌 원소만 저장하는 효율적인 저장 방식이 필요
0이 아닌 원소에 대해서만 연산을 수행하면 연산 속도를 크게 향상

그렇기에 다양한 분야에서 대용량 데이터를 효율적으로 처리하는 데 중요한 역할
'''
...
lambda 식을 사용해 강의명을 x.split()으로 나눈 뒤, [:4]
즉, 앞에서 4개까지만 텍스트를 가져오고 다시 join으로 합치기
이는 중복된 텍스트를 구분해서 보기 위함

빈도 수를 기준으로 내림차순으로 10개만 미리보기로 확인
'''
...
'''
3개의 n-gram과 빈도 수로 역순 정렬을 하게 되면 빈도 수가 높고, n-gram 수가 많은 순으로 정렬
drop_duplicates로 첫번째 강좌를 남기고 나머지 중복 삭제
'''
...
# 빈도 수로 정렬하고, 어떤 강좌가 댓글에서 가장 많이 언급되었는지 확인
...
#--- 6. TF-IDF로 가중치를 주어 벡터화​
'''
전체 문서에 자주 등장하지 않더라도 해당 문서에 자주 등장하는 단어라면 중요한 단어
TF-IDF 가중치를 적용하기 위해 TfidfTransformer()를 사용
TfidfVectorizer() = CountVectorizer() + TfidfTransformer()

따로 사용하는 이유는 빈도 기반의 BoW와 TF-IDF 가중치를 적용한 값을 비교하기 위함
'''
...
# 각 Row에서 전체 단어 가방 모형에 등장하는 단어에 대해 one-hot-vector에 TF-IDF 가중치를 반영한 결과 확인
tfidf_freq = pd.DataFrame(feature_tfidf.toarray(), columns=vocab)

'''
sum()으로 tfidf_freq의 합계를 만듦
그 이유는 TF-IDF 가중치를 적용하더라도 희소 행렬이 만들어지기에
각 피처마다 가중치가 제대로 적용되었는지 확인하기 위해
'''
...
#--- 7. 군집화
'''
KMeans는 머신러닝의 비지도학습 기법 중 하나
주어진 데이터를 K개로 묶는 알고리즘으로 군집 간 거리 차이의 분산을 최소화하는 방식으로 군집화

데이터 집합에서 K개의 데이터 개체를 임의로 추출하고 각 클러스터의 중심점을 초깃값으로 설정

K개의 군집과 데이터 집합의 개체 거리를 구해 각 개체가 어느 중심점과 가장 유사도가 높은지를 계산
찾은 중심점으로 다시 데이터 군집의 중심점을 계산하는 방법을 반복 (유클리드 거리!)

특징
    알고리즘이 단순하고 구현이 쉬움
    대용량 데이터에도 비교적 빠르게 동작
    
    클러스터의 개수 k를 미리 지정
    초기 중심 설정에 따라 결과가 달라질 수 있음
    클러스터 모양이 원형에 가까울 때 효과적
    이상치에 민감
    
활용 분야
    이미지 분할: 이미지를 여러 개의 영역으로 분할하는 데 사용
    고객 분류: 고객을 여러 그룹으로 나누어 마케팅 전략을 수립하는 데 사용
    문서 클러스터링: 문서를 주제별로 분류하는 데 사용
    이상 감지: 정상 데이터와 거리가 먼 이상 데이터를 탐지하는 데 사용
    
고려 사항
    적절한 k값 선택: 엘보우, 실루엣 분석 등 다양한 방법으로 최적의 k값 선택
    초기 중심 설정: k-means++ 알고리즘과 같은 초기 중심 설정 방법을 사용해 초기 중심 설정의 영향 감소시킬 수 있음
    
    데이터 전처리: 데이터의 스케일링, 이상치 제거 등 전처리를 통해 성능 향상
    
작동 방식
    1. 초기 중심 설정: k개의 클러스터 중심을 임의로 설정
    2. 클러스터 할당: 각 데이터 포인트를 가장 가까운 클러스터 중심에 할당
    3. 중심 재계산: 각 클러스터의 중심을 해당 클러스터에 속한 데이터 포인트들의 평균으로 재계산
    4. 2번과 3번을 반복하며 클러스터 중심이 더 이상 변하지 않거나 최대 반복 횟수에 도달할 때까지 반복
'''
...
엘보우(Elbow) 기법
    ...
    현실 세계 데이터를 다루다보면, 이론처럼 급격하게 꺾이는 지점이 나타나지 않을 수 있음
    클러스터의 수가 너무 많으면 군집화 값을 관리하기 어려울 수도
'''
...
# MiniBatchKMeans
'''
데이터가 많다면 군집화에 속도가 오래 걸림
배치 사이즈를 지정해서 군집화를 진해하면 조금 빠르게 작업
'''

# 적절한 클러스터의 개수를 알기 위해 이너셔 값을 구함
...
plt.show()
'''
이너셔 값을 시각화하는 이유는 엘보 기법을 사용하기 위해서
y축의 이너셔 값은 가장 가까운 군집의 중심과 샘플의 거리 제곱합을 의미
    이 이너셔 값이 작을수록 군집이 잘 되었다고 평가
    중심과 샘플의 거리가 가까울수록 군집이 잘 되었다고 볼 수 있기 때문
    
반복문을 통해 이너셔 값을 구할 때 첫 군집은 랜덤하게 군집의 중심을 설정하기 때문에 높은 값이 나올 수 밖에
다음 군집부터는 거리를 계산해 군집의 중심을 이동하므로 중심과 군집은 점점 가까워지고 값이 점점 낮아지는 그래프

전처리가 잘된 정형 데이터라면 팔꿈치처럼 급격하게 꺾이는 부분이 이상적으로 등장할 수 있지만,
비정형 텍스트 데이터에서는 이론처럼 급격하게 꺾이는 부분이 나타나지 않을 수도
'''
plt.figure(figsize=(15, 4))

x_values = list(range(start, start + len(silhouettes[start:end])))
plt.plot(x_values, silhouettes[start:end], marker='o')  
...
실루엣 점수 활용
    최적의 클러스터 개수 선택: 다양한 클러스터 개수에 대해 실루엣 점수를 계산하고, 가장 높은 점수를 가지는 클러스터 개수를 선택
    클러스터링 알고리즘 성능 평가: 다양한 클러스터링 알고리즘의 성능 비교하는 데 사용
...
df.loc[df['bcluster'] == 24, ['bcluster', 'cluster', 'origin_text', 'course']].tail(3)

#--- 9. 클러스터 예측 평가
'''
분류, 회귀 모델은 지도학습으로, 정답이 있기 때문에 정답과 예측값을 비교 가능 
하지만 군집화는 비지도학습으로, 정답이 없기 때문에 목적에 따라 평가 방법을 정해야함
...
'''
unique(): 예측한 클러스터의 유일값을 구하기
where(prediction==label): 예측한 값이 클러스터 번호와 일치하는 것을 가져오기
mean(): 클러스터의 평균 값을 구하기
np.argsort(x_means)[::-1][:n_clusters]: 값을 역순으로 정렬해서 클러스터 수만큼 가져오기
'''
...
'''
점수가 클수록 예측 정확도가 높음
MiniBatchKMeans로 예측한 값을 기준으로 정렬해, 각 클러스터에서 점수가 높은 단어 추출
'''
